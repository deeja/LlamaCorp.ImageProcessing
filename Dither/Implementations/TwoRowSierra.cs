...
{
  [EnumLink(typeof(DitherAlgorithm), (int)DitherAlgorithm.TwoRowSierra)]
  public class TwoRowSierra : IDither
  {
    public void PerformWork(Image image, Parameters parameters = null)
    {
      ...
      for (var x = 0; x < image.Width; x++)
      {
        for (var y = 0; y < image.Height; y++)
        {
          ...
          var errorMultiplier = ((double)error / 16.0);

          // x + 1, y
          image.SetPixelError(x + 1, y, errorMultiplier * 4);

          // x + 2, y
          image.SetPixelError(x + 2, y, errorMultiplier * 3);

          // x - 2, y + 1
          image.SetPixelError(x - 2, y + 1, errorMultiplier * 1);

          // x - 1, y + 1
          image.SetPixelError(x - 1, y + 1, errorMultiplier * 2);

          // x, y + 1
          image.SetPixelError(x, y + 1, errorMultiplier * 3);
